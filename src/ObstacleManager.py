...
class ObstacleManager(object):
	...
	def get_state_validity(self, config):
		...
		# Out of Bounds check
		if (mapConfig[0] < 0 or mapConfig[0] >= self.mapWidth) or (mapConfig[1] < 0 or mapConfig[1] >= self.mapHeight):
			return False

		half_width = self.robotWidth // 2
		half_length = self.robotLength // 2
		corners = [
			(mapConfig[0] - half_width, mapConfig[1] - half_length), 
			(mapConfig[0] + half_width, mapConfig[1] - half_length), 
			(mapConfig[0] + half_width, mapConfig[1] + half_length), 
			(mapConfig[0] - half_width, mapConfig[1] + half_length)
		]
		...
		for x, y in corners:
			if x < 0 or x >= self.mapWidth or y < 0 or y >= self.mapHeight: # Corner Out of Bounds check
				return False
			if self.mapImageBW[y, x] == 1:  # Collision
				return False
	...
	def discretize_edge(self, config1, config2):
		list_x, list_y = [], []
		edgeLength = numpy.sqrt((config2[0]-config1[0])**2 + (config2[1]-config1[1])**2)
		# -----------------------------------------------------------
		# YOUR CODE HERE
		# -----------------------------------------------------------
